export const CppLogo = () => {
    return (
        <div>
            <svg 
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 50 50" 
            width="30px" 
            fill="#faebd7" >
                <path d="M44.453 36c.337-.58.547-1.233.547-1.82V13.82c0-.587-.21-1.24-.547-1.82L23.5 24z" />
                <path d="M25.362 47.56l17.776-10.18c.512-.294.978-.8 1.315-1.38L23.5 24 2.547 36c.338.58.803 1.086 1.315 1.38l17.776 10.18c1.024.587 2.7.587 3.724 0z" />
                <path d="M44.453 12c-.337-.58-.803-1.086-1.315-1.38L25.362.44c-1.024-.587-2.7-.587-3.724 0L3.862 10.62C2.838 11.207 2 12.647 2 13.82v20.36c0 .587.21 1.24.547 1.82L23.5 24z" />
                <g fill="#000">
                    <path d="M23.5 38.218C15.597 38.218 9.167 31.84 9.167 24S15.597 9.782 23.5 9.782c5.1 0 9.856 2.722 12.413 7.105l-6.204 3.56a7.208 7.208 0 00-6.21-3.556c-3.95 0-7.166 3.189-7.166 7.109s3.215 7.11 7.167 7.11c2.55 0 4.93-1.364 6.21-3.557l6.203 3.56c-2.556 4.383-7.313 7.105-12.413 7.105z" />
                    <path d="M37.833 23.21h-1.592v-1.58h-1.593v1.58h-1.592v1.58h1.592v1.58h1.593v-1.58h1.592zM43.806 23.21h-1.593v-1.58H40.62v1.58h-1.592v1.58h1.592v1.58h1.593v-1.58h1.593z" />
                </g>
            </svg>
        </div>
    )
}

export default CppLogo
